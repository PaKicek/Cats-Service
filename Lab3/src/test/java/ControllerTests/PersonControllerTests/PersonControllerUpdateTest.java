...
public class PersonControllerUpdateTest {
    ...
    public void updatePersonTest() throws Exception {
        ...
        this.mockMvc.perform(put("/api/persons/put/3").with(user("username").password("password").roles("USER", "ADMIN")).contentType(MediaType.APPLICATION_JSON).content(toJson(personDto))).andExpect(status().isOk());
        verify(personService, times(1)).update(any(PersonRequest.class), eq(3L));
    }
}
