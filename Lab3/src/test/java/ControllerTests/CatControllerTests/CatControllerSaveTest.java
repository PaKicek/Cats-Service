...
public class CatControllerSaveTest {
    @Autowired
    ...
    public void saveCatTest() throws Exception {
        ...
        this.mockMvc.perform(post("/api/cats/save").with(user("username").password("password").roles("USER", "ADMIN")).contentType(MediaType.APPLICATION_JSON).content(toJson(catRequest))).andExpect(status().isOk());
        verify(catService, times(1)).save(any(CatRequest.class));
    }
}
