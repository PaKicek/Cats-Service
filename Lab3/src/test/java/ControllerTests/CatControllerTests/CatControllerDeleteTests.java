...
public class CatControllerDeleteTests {
    @Autowired
    ...
    @Test
    public void deleteCatByIdTest() throws Exception {
        this.mockMvc.perform(delete("/api/cats/delete/3").with(user("username").password("password").roles("USER", "ADMIN"))).andExpect(status().isOk());
        verify(catService, times(1)).deleteById(3);
    }
    @Test
    public void deleteAllCatsTest() throws Exception {
        this.mockMvc.perform(delete("/api/cats/deleteall").with(user("username").password("password").roles("USER", "ADMIN"))).andExpect(status().isOk());
